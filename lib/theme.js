...
Theme.prototype.index = function(posts, onDone) {
  ...
  miniq(1, paginate(posts, 5).map(function(item, index) {
    ...
    return function(done) {
      ...
      self._renderFn(self._templatePaths.index, locals, function(err, html) {
        if (!err && html) {
          result.push(html);
        }
        done(err);
...
Theme.prototype.author = function(author, posts, onDone) {
  ...
  miniq(1, paginate(posts, 5).map(function(item, index) {
    return function(done) {
      ...
      self._renderFn(self._templatePaths.author, locals, function(err, html) {
        if (!err && html) {
          result.push(html);
        }
        done(err);
  ...
  Object.keys(tags).forEach(function(tag) {
    ...
    paginate(posts, 5).forEach(function(item, index) {
      ...
      tasks.push(function(done) {
        self._renderFn(self._templatePaths.tag, locals, function(err, html) {
          if (!err && html) {
            if (!result[tag]) {
              result[tag] = [html];
            } else {
