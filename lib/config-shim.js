...
module.exports = function getConfig(meta) {
  var config = {
    ...
    paths: {
      subdir: meta.blog.subdir || '', // if the blog is not hosted at the root
      availableThemes: {
        casper: {
  ...
  config.urlFor = function urlFor(context, data, absolute) {
    ...
    if (_.isObject(context) && context.relativeUrl) {
        ...
        if (context === 'post' && data.post) {
            var relUrl = data.post.preComputedRelativeUrl;
            var reg = /(index.html)$|(.html)$/g;
            relUrl = relUrl.replace(reg, function (match, p1, p2) {
                if (p1) { return '' };
                return '/'
            })
            urlPath = relUrl;
